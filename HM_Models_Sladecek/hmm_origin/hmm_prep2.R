...
hmm_prep = function(formula, data, loo = F,delta_init=NULL,tpm_init=NULL,means_init=NULL,covar_init=NULL,...) {
  # convert formula
  if (missing(formula) & any(is.null(delta_init),is.null(tpm_init),is.null(means_init),is.null(covar_init))) {
    stop("You have to provide a formula!")
  } else if (class(formula) != "formula") {
    formula = as.formula(formula)
  }
  if(length(formula)==2 & any(is.null(delta_init),is.null(tpm_init),is.null(means_init),is.null(covar_init))){
    stop("You have to provide either response variable for parameter estimation, or a complete set of initial parameters!")
  }
  ...
  return(output)
    ...
    states=1:length(delta_init)
    predictors= all.vars(formula)
    sequences=1
    means=array(data = as.numeric(means_init),dim = c(1,length(predictors),length(states)),
                dimnames = list(sequences=1,predictors=predictors,states=states))
    
    covs=array(data = as.numeric(covar_init),dim = c(length(predictors),
                                length(predictors),
                                length(states)),
               dimnames = list(sequences=rep(1,each=length(predictors)),
    ...
    tpm=matrix(data = tpm_init, ncol=length(states),nrow=length(states))
    tpm=cbind(tpm,Seq=rep(unique(sequences),each=length(states)))
    deltas=delta_init
    output=list(delta=deltas, means=means,covariances=covs,tpm=tpm,
                formula=formula,data=data,predictors=predictors,sequences=1,
                states=states)
    class(output)="hmmodel"
    return(output)
  }
