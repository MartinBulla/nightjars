...
inspector.LOGGERS <- function(x, ...){
  list(
    ...
    x[ data != 'no' & logger_type %in% c('TTn','TTs','TTa'), .(TT_file_present, rowid)] %>%
    is.element_validator(v = data.table(variable = "TT_file_present",
    set = list(c('TRUE'))), "TT file not found in DATA folder; mistyped or missing") ,

    x[ data != 'no' & logger_type %in% c('HOBO'), .(HOBO_file_present, rowid)] %>%
    is.element_validator(v = data.table(variable = "HOBO_file_present",
    set = list(c('TRUE'))), "HOBO file not found in DATA folder; mistyped or missing") ,

    x[ data != 'no' & logger_type == 'DHT', .(DHT_file_present, rowid)] %>%
    is.element_validator(v = data.table(variable = "DHT_file_present",
    set = list(c('TRUE'))), "DHT file not found in DATA folder; mistyped or missing")   
  )
}
